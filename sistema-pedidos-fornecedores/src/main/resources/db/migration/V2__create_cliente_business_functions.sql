...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM clientes WHERE usuario = p_usuario) THEN
        RAISE EXCEPTION 'Regra de Negócio Violada: Nome de usuário já existe (%)', p_usuario;
    END IF;

    INSERT INTO clientes (nome, cpf, data_nascimento, usuario, senha, saldo)
    VALUES (p_nome, p_cpf, p_data_nascimento, p_usuario, p_senha_hash, 0.00)
