...
CREATE TABLE pedidos (
    ...
    valor_desconto DECIMAL(19, 2) NOT NULL DEFAULT 0.00,
    ...
    CONSTRAINT fk_pedidos_cupons FOREIGN KEY (cupom_id) REFERENCES cupons(id)
);
...
CREATE TABLE transacoes (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    data_transacao TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT CURRENT_TIMESTAMP,
    tipo_transacao VARCHAR(100) NOT NULL,
    valor DECIMAL(19, 2) NOT NULL,
    cliente_id UUID NOT NULL,
    pedido_id UUID,
    CONSTRAINT fk_transacoes_clientes FOREIGN KEY (cliente_id) REFERENCES clientes(id),
    CONSTRAINT fk_transacoes_pedidos FOREIGN KEY (pedido_id) REFERENCES pedidos(id)
);