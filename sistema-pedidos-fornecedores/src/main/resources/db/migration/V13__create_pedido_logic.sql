-- Função principal para realizar um pedido de forma atômica
...
DECLARE
    ...
    novo_pedido_id UUID;
BEGIN
    -- 1. CALCULAR VALORES E VALIDAR PRODUTOS
    FOR item IN SELECT * FROM jsonb_to_recordset(p_itens_pedido_json) AS x(produto_id UUID, quantidade INT)
    LOOP
        -- Valida se o produto existe e está ativo
        SELECT preco, percentual_desconto INTO produto_info FROM produtos WHERE id = item.produto_id AND ativo = TRUE;
        IF NOT FOUND THEN
            RAISE EXCEPTION 'Produto com ID % não encontrado ou está inativo.', item.produto_id USING ERRCODE = 'PPE02';
        END IF;
    ...
    -- 2. VALIDAR E APLICAR CUPOM (se houver)
    IF p_codigo_cupom IS NOT NULL AND p_codigo_cupom <> '' THEN
        ...
        IF cupom_info.tipo_desconto_cupom = 'P' THEN -- Percentual
    ...
    -- 3. VALIDAR E DEBITAR SALDO DO CLIENTE
    ...
    -- 4. REGISTRAR PEDIDO E TRANSAÇÃO
    ...
    -- 4.1 Inserir itens do pedido
    INSERT INTO itens_pedido (pedido_id, produto_id, quantidade, preco_unitario_cobrado)
    SELECT novo_pedido_id, (item_data->>'produto_id')::UUID, (item_data->>'quantidade')::INT,
           (SELECT preco * (1 - percentual_desconto / 100.0) FROM produtos WHERE id = (item_data->>'produto_id')::UUID)
    FROM jsonb_array_elements(p_itens_pedido_json) AS item_data;

    -- 4.2 Registrar transação de pagamento
    ...
    -- 5. INCREMENTAR USO DO CUPOM (se houver)
